...
//     const formattedMonth = String(currMonth + 1).padStart(2, "0"); // Ensure month is two digits
//     const formattedYear = String(currYear); // Year remains as is
//     const formattedSelectedDate = `${formattedDay}/${formattedMonth}/${formattedYear}`;
...
//           <span
//             key={`active-${i}`}
//             className={`py-[2px] font-light ${isToday? (isSelected ? "bg-teal-300 rounded-full w-6 h-6 text-teal-800": "bg-teal-500 rounded-full w-6 h-6 text-white"): ""} ${isSelected ? "bg-teal-500 text-white w-6 h-6 rounded-full" : ""} ${
...
//         <p className="text-lg text-gray-900 font-semibold">{`${months[currMonth]} ${currYear}`}</p>
//         <div className="flex">
//           <span
...
//                 <div
//                   className="w-6 h-6 flex items-center justify-center rounded-full text-gray-700"
//                   // Add additional styles based on the day's status (e.g., selected, today)
//                 >
//                   {day}
//                 </div>
//               </div>
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= lastDateOfMonth; i++) {
      const isToday =
        ...
        currMonth === new Date().getMonth() &&
        currYear === new Date().getFullYear();
      const isSelected = new Date(currYear, currMonth, i) && i === selectedNum;
      const isPastDate =
        new Date(currYear, currMonth, i) < new Date().setHours(0, 0, 0, 0);
      ...
      days.push(
        <span
          ...
          className={`py-[2px] font-light ${
            isToday
              ? isSelected
                ? "bg-teal-300 rounded-full w-6 h-6 text-teal-800"
                : "bg-teal-500 rounded-full w-6 h-6 text-white"
              : ""
          } ${
